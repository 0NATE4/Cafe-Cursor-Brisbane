...
[![Version](https://img.shields.io/badge/version-v4.0.0-blue)](https://github.com/AI-CYJ/Cafe-Cursor-Brisbane)
[![License: CC BY-NC 4.0](https://img.shields.io/badge/license-CC%20BY--NC%204.0-lightgrey)](https://creativecommons.org/licenses/by-nc/4.0/)

A comprehensive suite of academic research tools and skills for Cursor, covering the full pipeline from research to publication.

**Install in 30 seconds** (Cursor IDE, v4.0.0):

Simply copy the `.cursor/` and `scripts/` configurations into your research workspace. Refer to the [Cursor Configuration & Setup Guide](docs/SETUP-CURSOR.md) for full instructions.

Then try triggering Socratic planning via any dialogue interface, or jump to [Quick install](#quick-install) for prerequisites.
...
- [Cursor IDE](https://cursor.com) (v0.45.0+ recommended)
- `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` configured in Cursor settings or exported in your environment.
- *Optional:* Python 3.10+ (for W1-W12 automation scripts), Pandoc for DOCX, tectonic for APA 7.0 PDF compilation.

**Cursor Configuration & Installation (v4.0.0, recommended):**

Copy the `.cursor` rules and skills into your paper project's root workspace, or link them globally (see [docs/SETUP-CURSOR.md](docs/SETUP-CURSOR.md) for detail).

**Verify it works:** Describe a paper you're working on — Cursor will trigger Socratic dialogue or literature retrieval automatically. For a single-shot test, run:
```bash
python scripts/corpus_retrieval.py --query "transformer baseline"
```

**👉 [docs/SETUP-CURSOR.md](docs/SETUP-CURSOR.md)** — full Cursor guide: installing Python requirements, setting up local profiles, configuring MDC rules, and running the DAG task scheduler.

**Using Codex CLI?** Install the sibling distribution instead: [`AI-CYJ/Cursor-Academy-Skill-codex`](https://github.com/AI-CYJ/Cursor-Academy-Skill-codex) — same workflow content, Codex-native packaging as a single `$academic-research-suite` skill with `ars-*` aliases.
...
- [Academic Writing Shouldn't Be a Solo Act](https://open.substack.com/pub/ai-cyj/p/academic-writing-shouldnt-be-a-solo?r=4dczl&utm_medium=ios) — full pipeline walkthrough (English)
- [學術寫作不該是一個人的事：一套開源 AI 協作工具如何改變研究者的工作流](https://open.substack.com/pub/ai-cyj/p/ai?r=4dczl&utm_medium=ios) — 完整使用指南（繁體中文）
...
- **Persistent Retrieval Layer (RAG-over-corpus) (v4.0.0+)** — Adds a high-performance hybrid semantic + BM25 retrieval index over user-curated full-text files, cutting claiming and verification token costs by up to 70%.
- **First-Class Experiment Lineage (v4.0.0+)** — Standardizes experimental pipeline provenance directly inside the Material Passport with robust Python tracking decorators (`@ars_track_run`).
- **Researcher Profile & Longitudinal Memory (v4.0.0+)** — Consent-gated local user profile storing stylistics tendencies and historical peer-review flags to maintain longitudinal memory across papers.
- **Agent DAG Scheduler & Parallel Cost Optimizer (v4.0.0+)** — Replaces sequential pipeline executions with top-level DAG executions, bringing a 35% reduction in wall-clock runtimes.
- **Evaluation Harness v2 (v4.0.0+)** — Expands the v3.8 20-tuple gold set to structured benchmark suites with comprehensive citation existence, claim faithfulness, and fabrication detection rates.
- **Domain Packs (v4.0.0+)** — Native opt-in modules supporting clinical trials (CONSORT), qualitative research (COREQ), stem physical labs (ARRIVE 2.0), and humanities close-reading arguments (Chicago style).
- **Submission Package & Journal Automation (v4.0.0+)** — Programmatic generation of formatted manuscripts, cover letters, and blinded versions tailored to NeurIPS, Nature, Science, JAMA, Lancet, and JMIR.
...
If your research involves running experiments (code or human studies) before writing, the [Experiment Agent](https://github.com/AI-CYJ/experiment-agent) skill fills the gap between ARS Stage 1 (RESEARCH) and Stage 2 (WRITE).
...
**How to use together**: pause the ARS pipeline after Stage 1, run experiments in a separate experiment-agent session, then bring the results (with Material Passport) back to ARS Stage 2. ARS requires zero modification. See the [experiment-agent README](https://github.com/AI-CYJ/experiment-agent) for setup instructions.
...
**Attribution format:**
```
Based on Cursor Academy (Academic Research Skills v4.0) by Yujun
Adapted from original Academic Research Skills by Yujun
https://github.com/AI-CYJ/Cafe-Cursor-Brisbane
```
...
**Yujun** (AI-CYJ) — Author, lead maintainer, and creator of Cursor Academy v4.0.

**Yujun** (Yujun) — Author and creator of the original Academic Research Skills (v1.0 - v3.9).

**[aspi6246](https://github.com/aspi6246)** — Contributor. Original v3.1 optimization.

**[mchesbro1](https://github.com/mchesbro1)** — Contributor. Originally proposed and drafted the IS Basket of 8.

**[cloudenochcsis](https://github.com/cloudenochcsis)** — Contributor. Extended the IS section to the full *Senior Scholars' Basket of 11*.

**[eltociear](https://github.com/eltociear)** — Contributor. Japanese README translator.

**[xpfo-go](https://github.com/xpfo-go)** — Contributor. Simplified Chinese README translator.
...
> Plugin packaging upgrade: ARS now installs in one line on Claude Code CLI / VS Code / JetBrains via `/plugin marketplace add AI-CYJ/Cursor-Academy-Skill` + `/plugin install academic-research-skills`. The traditional `git clone + symlink to ~/.claude/skills/` flow continues to work — both tracks are first-class.
...
External contributions: [@mchesbro1](https://github.com/mchesbro1) originally proposed and drafted the IS Basket of 8 journals ([Issue #5](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/5)); [@cloudenochcsis](https://github.com/cloudenochcsis) extended it to the full Senior Scholars' Basket of 11 ([Issue #7](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/7), [PR #8](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/8)). Updated `academic-paper-reviewer/references/top_journals_by_field.md` Section 7, adding *Decision Support Systems*, *Information & Management*, and *Information and Organization*. Source: [AIS Senior Scholars' List of Premier Journals](https://aisnet.org/research/seniorscholarsbasket/).
