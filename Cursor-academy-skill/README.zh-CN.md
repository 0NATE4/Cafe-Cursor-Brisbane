...
一套完整的学术研究 Cursor 专属技能包，涵盖从研究到论文出版的全流程。

**30 秒安装**（Cursor IDE，v4.0.0）：

只需将 `.cursor/` 规则与 `scripts/` 配置复制到您的研究工作区根目录中。详细安装指南请参考 [Cursor 配置与安装指南](docs/SETUP-CURSOR.md)。

安装后，您可以在 Cursor Chat、Composer 或 AI 辅助模式中直接描述研究构想，系统将自动触发学术 RAG 检索或苏格拉底式对话。
...
- [Cursor IDE](https://cursor.com)（建议 v0.45.0+）
- 在 Cursor 中配置 `ANTHROPIC_API_KEY` 或 `OPENAI_API_KEY`，或者在系统环境变量中导出。
- *选用：* Python 3.10+（用于 W1-W12 自动化脚本），Pandoc 用于 DOCX 输出，tectonic 用于 APA 7.0 PDF 编译。

**Cursor 配置与安装（v4.0.0，推荐）：**

将 `.cursor` 规则和技能复制到您的论文项目根工作区，或者进行全局链接（详见 [docs/SETUP-CURSOR.md](docs/SETUP-CURSOR.md)）。

**验证可用：** 描述你正在写的论文 — Cursor 会自动触发学术 RAG 检索或苏格拉底式对话。若要进行单次测试，可以运行：
```bash
python scripts/corpus_retrieval.py --query "transformer baseline"
```

**👉 [docs/SETUP-CURSOR.md](docs/SETUP-CURSOR.md)** — 完整 Cursor 指南：安装 Python 前置依赖、配置本土地理特征 profile、设置 MDC 规则，以及运行 DAG 任务调度器。

**用 Codex CLI？** 请安装姐妹版：[`AI-CYJ/Cursor-Academy-Skill-codex`](https://github.com/AI-CYJ/Cursor-Academy-Skill-codex)。同一套 workflow 内容，Codex 原生打包为单一 `$academic-research-suite` skill，提供 `ars-*` 别名。
...
- [学术写作不该是一个人的事：一套开源 AI 协作工具如何改变研究者的工作流](https://open.substack.com/pub/ai-cyj/p/ai?r=4dczl&utm_medium=ios) — 完整使用指南（繁体中文）
...
如果你的研究需要在写作前做实验（代码或人工研究），[Experiment Agent](https://github.com/AI-CYJ/experiment-agent) 技能填补 ARS Stage 1（研究）和 Stage 2（写作）之间的空缺。
...
**搭配使用方式**：ARS pipeline 完成 Stage 1 后暂停，在另一个 experiment-agent session 中执行实验，完成后将结果（含 Material Passport）带回 ARS Stage 2。ARS 不需要任何修改。详见 [experiment-agent README](https://github.com/AI-CYJ/experiment-agent)。
...
**[mchesbro1](https://github.com/mchesbro1)** — 贡献者。最初提出并撰写了 IS Basket of 8 期刊清单（[Issue #5](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/5)）。

**[cloudenochcsis](https://github.com/cloudenochcsis)** — 贡献者。将 IS 章节从 *Basket of 8* 扩充为完整的 *Senior Scholars' Basket of 11*，补上 *Decision Support Systems*、*Information & Management*、*Information and Organization*（[Issue #7](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/7)、[PR #8](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/8)）。数据源：[AIS Senior Scholars' List of Premier Journals](https://aisnet.org/research/seniorscholarsbasket/)。

**[eltociear](https://github.com/eltociear)**（Ikko Eltociear Ashimine）— 贡献者。翻译了日文版 README（[`README.ja-JP.md`](README.ja-JP.md)）（[PR #161](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/161)）。

**[xpfo-go](https://github.com/xpfo-go)**（xpfo）— 贡献者。翻译了简体中文版 README（[`README.zh-CN.md`](README.zh-CN.md)）（[PR #181](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/181)）。
...
> Plugin 打包升级：ARS 现可在 Claude Code CLI / VS Code / JetBrains 一行装（`/plugin marketplace add AI-CYJ/Cursor-Academy-Skill` + `/plugin install academic-research-skills`）。原本的 `git clone + symlink 到 ~/.claude/skills/` 安装流程不变、继续支持；双轨都是一级公民。
...
外部贡献：[@mchesbro1](https://github.com/mchesbro1) 最初提出并撰写了 IS Basket of 8 期刊清单（[Issue #5](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/5)）；[@cloudenochcsis](https://github.com/cloudenochcsis) 将其扩充为完整的 Senior Scholars' Basket of 11（[Issue #7](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/7)、[PR #8](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/8)）。更新 `academic-paper-reviewer/references/top_journals_by_field.md` 第 7 节，补上 *Decision Support Systems*、*Information & Management*、*Information and Organization*。数据源：[AIS Senior Scholars' List of Premier Journals](https://aisnet.org/research/seniorscholarsbasket/)。
