...
一套完整的學術研究 Cursor 專屬技能包，涵蓋從研究到論文出版的全流程。

**30 秒安裝**（Cursor IDE，v4.0.0）：

只需將 `.cursor/` 規則與 `scripts/` 配置複製到您的研究工作區根目錄中。詳細安裝指南請參考 [Cursor 配置與安裝指南](docs/SETUP-CURSOR.md)。

安裝後，您可以在 Cursor Chat、Composer 或 AI 輔助模式中直接描述研究構想，系統將自動觸發學術 RAG 檢索或蘇格拉底式對話。
...
- [Cursor IDE](https://cursor.com)（建議 v0.45.0+）
- 在 Cursor 中配置 `ANTHROPIC_API_KEY` 或 `OPENAI_API_KEY`，或者在系統環境變數中匯出。
- *選用：* Python 3.10+（用於 W1-W12 自動化腳本），Pandoc 用於 DOCX 輸出，tectonic 用於 APA 7.0 PDF 編譯。

**Cursor 配置與安裝（v4.0.0，推薦）：**

將 `.cursor` 規則和技能複製到您的論文項目根工作區，或者進行全域連結（詳見 [docs/SETUP-CURSOR.md](docs/SETUP-CURSOR.md)）。

**驗證可用：** 描述你正在寫的論文 — Cursor 會自動觸發學術 RAG 檢索或蘇格拉底式對話。若要進行單次測試，可以運行：
```bash
python scripts/corpus_retrieval.py --query "transformer baseline"
```

**👉 [docs/SETUP-CURSOR.md](docs/SETUP-CURSOR.md)** — 完整 Cursor 指南：安裝 Python 前置依賴、配置本地地理特徵 profile、設置 MDC 規則，以及運行 DAG 任務調度器。

**用 Codex CLI？** 請改裝姊妹版：[`AI-CYJ/Cursor-Academy-Skill-codex`](https://github.com/AI-CYJ/Cursor-Academy-Skill-codex)。同一套 workflow 內容，Codex 原生包裝為單一 `$academic-research-suite` skill，提供 `ars-*` 別名。
...
如果你的研究需要在寫作前跑實驗（程式碼或人工研究），[Experiment Agent](https://github.com/AI-CYJ/experiment-agent) 技能填補 ARS Stage 1（研究）和 Stage 2（寫作）之間的空缺。
...
**搭配使用方式**：ARS pipeline 跑完 Stage 1 後暫停，在另一個 experiment-agent session 中跑實驗，完成後將結果（含 Material Passport）帶回 ARS Stage 2。ARS 不需要任何修改。詳見 [experiment-agent README](https://github.com/AI-CYJ/experiment-agent)。
...
**標示格式：**
```
Based on Academic Research Skills by Yujun
https://github.com/AI-CYJ/Cursor-Academy-Skill
```
...
**[mchesbro1](https://github.com/mchesbro1)** — 貢獻者。最初提出並撰寫了 IS Basket of 8 期刊清單（[Issue #5](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/5)）。

**[cloudenochcsis](https://github.com/cloudenochcsis)** — 貢獻者。將 IS 章節從 *Basket of 8* 擴充為完整的 *Senior Scholars' Basket of 11*，補上 *Decision Support Systems*、*Information & Management*、*Information and Organization*（[Issue #7](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/7)、[PR #8](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/8)）。資料來源：[AIS Senior Scholars' List of Premier Journals](https://aisnet.org/research/seniorscholarsbasket/)。

**[eltociear](https://github.com/eltociear)**（Ikko Eltociear Ashimine）— 貢獻者。翻譯了日文版 README（[`README.ja-JP.md`](README.ja-JP.md)）（[PR #161](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/161)）。

**[xpfo-go](https://github.com/xpfo-go)**（xpfo）— 貢獻者。翻譯了簡體中文版 README（[`README.zh-CN.md`](README.zh-CN.md)）（[PR #181](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/181)）。
...
> Plugin 打包升級：ARS 現可在 Claude Code CLI / VS Code / JetBrains 一行裝（`/plugin marketplace add AI-CYJ/Cursor-Academy-Skill` + `/plugin install academic-research-skills`）。原本的 `git clone + symlink 到 ~/.claude/skills/` 安裝流程不變、繼續支援；雙軌都是一級公民。
...
外部貢獻：[@mchesbro1](https://github.com/mchesbro1) 最初提出並撰寫了 IS Basket of 8 期刊清單（[Issue #5](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/5)）；[@cloudenochcsis](https://github.com/cloudenochcsis) 將其擴充為完整的 Senior Scholars' Basket of 11（[Issue #7](https://github.com/AI-CYJ/Cursor-Academy-Skill/issues/7)、[PR #8](https://github.com/AI-CYJ/Cursor-Academy-Skill/pull/8)）。更新 `academic-paper-reviewer/references/top_journals_by_field.md` 第 7 節，補上 *Decision Support Systems*、*Information & Management*、*Information and Organization*。資料來源：[AIS Senior Scholars' List of Premier Journals](https://aisnet.org/research/seniorscholarsbasket/)。
