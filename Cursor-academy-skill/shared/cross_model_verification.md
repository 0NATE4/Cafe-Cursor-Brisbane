...
You need API keys from at least one additional provider. ARS itself runs inside your local AI agent (like Cursor), so your primary model is always available.
...
In Cursor, you can test by asking:
```
...
In Cursor, the agent can use the Shell tool to make API calls:
