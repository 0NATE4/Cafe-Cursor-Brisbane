...
## Cursor Agent and Composer Settings

Since Cursor's Agent/Composer is fully local-first, you do not need to configure complex permission skipping flags or external plugins.
* **Skip Permissions / Autonomy:** Simply configure the Cursor Composer's Agent settings to "Yolo Mode" / "Allow always-run tools" if you want uninterrupted execution of DAG tasks and file write operations without manual popup approvals.
* **Long-Running Session Management:** Cursor automatically maintains context and long-running conversation threads. However, if context grows too large (e.g. over 150K tokens), it is recommended to reset the session and paste back the latest Material Passport JSON/YAML. The orchestrator will parse the complied history and seamlessly resume at your breakpoint.

## Long-running session management

The full academic pipeline is designed for human-in-the-loop execution, with mandatory user confirmation at every stage. In practice, a full run often spans hours to days — longer than standard prompt cache TTL (5 minutes). Two consequences:
...
3. Start a fresh Cursor session (session B) and paste `resume_from_passport=<hash>`. Optional overrides: `resume_from_passport=<hash> stage=<n> mode=<m>`.
