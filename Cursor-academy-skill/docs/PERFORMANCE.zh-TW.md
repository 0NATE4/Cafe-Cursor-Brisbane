...
## Cursor Agent 與 Composer 設定

由於 Cursor 的 Agent/Composer 採用完全本地優先（Local-first）的設計，您不需要配置複雜的權限跳過標記或安裝外部插件：
* **免除權限授權 / 自主執行：** 只需在 Cursor Composer 的 Agent 設定中啟用「Yolo Mode」（或「始終允許執行工具」），即可不中斷地在後台執行 DAG 學術管線與各類檔案讀寫操作。
* **長會話上下文管理：** Cursor 會自動為您維護長會話的上下文，但如果 context 累積過大（例如超過 150K tokens 導致模型推理變慢），建議重開新會話，並將最新的 Material Passport JSON/YAML 貼回，協調器會自動解析歷史紀錄並在先前的中斷點完美重啟。

## 建議 Claude Code 舊版設定（保留作歷史參考）
